                    ...
                    if showCheckmarks {
                       ...
                       //     .padding(.bottom, 60)
                        ...
                        if generaterSelection == 1{
                            NavigationLink(destination: session_Crazy8(items: items, sessionName: sessionName, userInputs: userInputs, checkedInput: checkedIndex != nil ? userInputs[checkedIndex!] : "None", displayedQuestion: dataManager.questions[currentIndex].text)) {
                                Text("next crazy")
                                       EmptyView() // Empty view to trigger navigation
                                  }
                        }
                        else{
                            NavigationLink(destination: Session_ReverseBrainstorming(items: items, sessionName: sessionName, userInputs: userInputs, checkedInput: checkedIndex != nil ? userInputs[checkedIndex!] : "None", displayedQuestion: dataManager.questions[currentIndex].text)) {
                                Text("next reverse")
                                       EmptyView() // Empty view to trigger navigation
                                  }
                        }
...
//                        NavigationLink("Next", destination: SummaryView1(userInputs: userInputs, checkedInput: checkedIndex != nil ? userInputs[checkedIndex!] : "None", displayedQuestion: dataManager.questions[currentIndex].text))
//                            .frame(width: 337, height: 39)
//                            .background(Color.orange)
//                            .foregroundColor(.white)
//                            .clipShape(RoundedRectangle(cornerRadius: 10))
...
@State private var timeRemaining = 1 * 60 // 8 minutes in seconds
    ...
    NavigationView {
        ZStack {
            ...
            VStack {
                ...
                NavigationLink(destination: crazy8Summary(problemStatement: problemStatement)) {
                    EmptyView()
                }
                .hidden()
                Button("Save") {
        ...
        NavigationStack{
            ZStack{
                ...
                VStack{
                    ...
                    NavigationLink(destination: BrainstormingSummary(problemStatement: problemStatement), isActive: $navigateSummary) {
                        EmptyView() // Empty view to trigger navigation
                    }
